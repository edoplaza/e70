$(document).ready( function($){
  ...
  if ( window.innerWidth > desktop ) {
	  ...
	  as.forEach((e, i)=> {
	  	e.on('click', document, ()=> {
		  	toggleNav();
		  	down.addClass('fadeOut');
		  	svgs.addClass('fadeOut');
		  	addLogoColor();
		  	content.animate({ top: -(flags[i + 3]) }, speed, "easeOutSine", function(){
		  		setTimeout( () => flag = 0, wait);
		  		counter = i + 3;
		  	});
		  });
	...
	if ( window.innerWidth <= desktop ) {
	  ...
	  as.forEach((e, i)=> {
	  	e.on('click', document, ()=> {
		  	toggleNav();
		  	content.animate( {scrollTop: flags[i + 3] }, 500, "easeOutSine");
		  });
		})
	}
